...
`EnsureThreadSafeTiles()` uses `PC::ParIterType`, whose iterator skips absent
entries and entries with zero real particles. The cross-container builder then
uses `other.ParticlesAt(lev, pti)`, which is documented and implemented with
`map::at`; a source tile missing from the target map therefore throws despite
the purported pre-creation pass.

## Quokka impact classification

**Classification: Definitely does not affect Quokka.**

`EnsureThreadSafeTiles` is used by AMReX's cross-container neighbor-list
builders. Quokka has no `NeighborParticleContainer`, does not build a
cross-container `NeighborList`, and contains no call to
`EnsureThreadSafeTiles`. Its interactions among particle types are dispatched
through the Quokka particle register and separate mesh deposition buffers,
not AMReX neighbor lists.

Accordingly no Quokka target map is prepared by this faulty helper and no
`ParticlesAt(...).at()` lookup depends on it. Integrating the neighbor-list
subsystem would be new functionality, so the exclusion is definite.
